...
const OX_QUESTIONS: OxQ[] = [
  ...
  { text: "폭력이 있을 때 경찰이나 이웃에 알리게 되면 창피하고 남부끄러운 일이므로 참는다. ", answer: "X" },
  { text: "부부관계라 처리가 안된다. 사실혼관계에 있는 사람은 법적인 조치가 더욱 안된다. ", answer: "X" },
  { text: "여자가 어떻게 남편을 고소하나, 아버지를 신고하는 자식은 호로자식이다. ", answer: "X" },
  { text: "가정폭력은 흔히 있을 수 있는 일이다.", answer: "X" },
  { text: "맞고 사는 아내에게 문제가 있다.", answer: "X" },
  { text: "가정폭력방지법은 성별과 나이 관계를 불문하고 가정에서 일어나는 신체·정서·언어·경제 등 모든 폭력에 적용된다.", answer: "O" },
  { text: "가정폭력 신고는 112 및 가까운 지구대, 파출소 등의 수사기관이나, 여성긴급전화 국번없이1366에 전화 또는 직접 찾아가 신고하면 된다.", answer: "O" },
  { text: "가정폭력 방지법은 처벌이 아니라 부부를 포함한 가족 간의 폭력을 제지하고 예방 교육하는 것이 목적이다.", answer: "O" },
  { text: "1998년 7월 1일부터 시행중인 가정폭력특례법은 가정폭력 방지 및 피해자 보호 등에 관한 법률과 가정폭력 범죄의 처벌 등에 관한 특례법이다.", answer: "O" },
  { text: "가정폭력을 피해 집을 나와 당장 집으로 돌아가기도 어렵고, 남의 도움을 받기도 어렵다면 보호시설을 이용하실 수 있다.", answer: "O" },
  { text: "가정폭력 범죄로 신고 되더라도 모두 형사처벌을 받아 전과자가 되는것은 아니며, 가정보호 사건으로 처리되는 경우에는 범죄경력이 남지 않는다.", answer: "O" },
  { text: "강제적으로 돈과 재산을 가져가거나 통제하며 일을 못하게 하고 경제권을 남편이 통제, 관리하는 등의 행위도 가정폭력이다.", answer: "O" },
  { text: "가정폭력은 우리 모두가 힘을 모아 뿌리 뽑아야 할 사회악이다.", answer: "O" },
  { text: "정서적인 학대도 폭력에 속한다. 즉, 모욕을 준다던지 굴욕과 비난, 무시, 욕설등도 포함된다. ", answer: "O" },
  ...
  { text: "아내가 아무리 남편에게 잘 해주어도 남편의 폭력은 중단될 수 없다. ", answer: "O" },
  { text: "비록 남편이 아내를 구타하기는 하지만, 정신적인 문제는 전혀 없다.  ", answer: "O" },
            ...
            {RANK.map((u, i) => {
              ...
              return (
                <li key={i} className="rank-item">
                  ...
                  <div className="avatar"><img src={getAvatar(u.name)} alt={`${u.name} 아바타`} /></div>
        ...
        <section className="q-card q-card--fixed">
          ...
          {mode === "quiz" && (
            <div className="q-body">
              ...
              <div className="q-choices">
                <button type="button" className="q-choice q-o" onClick={() => choose("O")} aria-label="O">O</button>
                <button type="button" className="q-choice q-x" onClick={() => choose("X")} aria-label="X">X</button>
              </div>
            </div>
          )}
          ...
          {mode === "feedback" && (
            <div className="q-body">
              ...
              <div className={`q-badge ${isCorrect ? "is-correct" : "is-wrong"}`}>
                {isCorrect ? "정답!" : "오답"}
              </div>
